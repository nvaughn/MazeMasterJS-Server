...
const mazeNeighborHash = '4c2172b185d3cb25c7ec916b9f11bba1';
...
describe('Maze', () => {
    ...
    describe('Maze.getCellNeighbor(Cell(1,1), DIRS.EAST)', () => {
        it('should return return cell object with MD5 hash value of ' + mazeNeighborHash + '', done => {
            let cPos = new Position(1, 1);
            let cell = maze.getCell(cPos);
            let cellNeighbor = maze.getNeighbor(cell, DIRS.EAST);
            assert.equal(mazeNeighborHash, md5(JSON.stringify(cellNeighbor)));
            done();
        });
    });
